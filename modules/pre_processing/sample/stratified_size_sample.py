...
def stratified_size_sample(params: dict):
    input1_dir = params['params.input.1.dir']
    print("input1_dir", input1_dir)
    df: DataFrame = pd.read_pickle(input1_dir)

    stratified_size: str = params['params.strataSizes']
    print("strataSizes", stratified_size)
    ...
    tmp_df_list = []
    for i in stratified_size.split(","):
        tuple2 = i.split(":")
        column = tuple2[0]
        size = int(tuple2[1])
        tmp_df_list.append(df2.get_group(column).sample(n=size, replace=replace.lower() == str(True).lower()))
